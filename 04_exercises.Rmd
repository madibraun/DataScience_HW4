...
favorites_madi <- tibble(
  name = c("Bayfield, WI", "Hometown", "3M", "The Naughty Greek", "Macalester College", "Current Home", "Mac-Groveland Ice Rink", "Minnehaha Falls", "PetSmart"),
...
dc_map <- get_stamenmap(
  bbox = c(left = -77.4215, bottom = 38.7343, right = -76.673, top = 39.0549),
  maptype = "terrain",
  zoom = 11
)
...
ggmap(dc_map) + 
  ...
  labs(x = "Longitude", y = "Latitude", color = "# of Departures")
```
...
ggmap(dc_map) + 
  geom_point(data = casual_depart,
             ...
             color = perccasual)) +
  scale_color_viridis_c(option = "viridis") +
  labs(x = "Longitude", y = "Latitude", color = "Percent Casual Users")
```
...
The data are not normalized by population size.
...
mpls_suspicious <- MplsStops %>% 
  ...
  pivot_wider(names_from = problem, values_from = numreport) %>% 
  mutate(propsusp = suspicious/sum(suspicious, traffic), total = sum(suspicious, traffic)) 
